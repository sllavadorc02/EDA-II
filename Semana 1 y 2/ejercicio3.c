...
/* Ejercicio 4: */
  ...
  char inorden[] = {'C', 'A', 'E', 'B', 'D', 'G', 'F'};
  char preorden[] = {'B', 'A', 'C', 'E', 'D', 'F', 'G'};
  int longitud = sizeof(inorden) / sizeof(inorden[0]);
  Arbol raizz = reconstruir(preorden, inorden, 0, longitud - 1, 0, longitud - 1);
  printf("\n\nSe ha reconstruido el arbol:\n");
  printf("Recorrido en PRE_ORDEN (prefija): ");
  preOrden(raizz);
  printf("\n");
  printf("Recorrido en ORDEN(infija): ");
